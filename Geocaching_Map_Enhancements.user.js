...
// @attribution   2011-18, James Inge (http://geo.inge.org.uk/)
...
var gmeResources = {
	...
	env: {
		...
		init: [],
		//loggedin: (!!document.getElementById("#gc-header .player-profile") || !!document.getElementById("#gc-header .player-profile")),
        loggedin: true,
		page: "default",
		storage: false,
		...
		common: function () {
			...
			function setEnv() {
				...
				case "maps":
					...
					if (typeof L === "object" && typeof $ === "function" && window.MapSettings && window.MapSettings.Map && window.MapSettings.Map._loaded && $(".leaflet-control-layers, .map-setting-controls ul").length > 0) {
						gmeInit(gmeConfig.env.init);
						window.setTimeout(load,500);
						return;
					}
